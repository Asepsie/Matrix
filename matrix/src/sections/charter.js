/* ►► SECTION: CHARTER ◄◄ Cross-functional project charter overlay.
 *   openCharterHub       — WORK › Charters rail view: lists every project
 *   closeCharterHub      — close the hub (wrapped by railnav to sync activeView)
 *   openCharter(projId)  — open the charter for a project (ctx menu or hub)
 *   chtClose             — close the editor overlay
 *   chtShowTab(tab)      — switch tab and render it
 *   chtSaveNow           — persist + toast
 *
 * 4 tabs: overview (business case + revenue) / demands (the stakeholder demands
 * board) / financials (live NPV·IRR·… via calculateFinancials) / decision (the
 * trade-off SQUARE + non-negotiables). Edits write straight into project.charter
 * and autosave via saveState().
 *
...
let _chtProjId = null;   // project currently open in the charter overlay
let _chtTab    = 'overview';

// Tab registry: id → label. Order defines the tab strip.
const CHT_TABS = [
  ['overview',   'OVERVIEW'],
  ['demands',    'DEMANDS'],
...
export function chtClose(){ G('cht-overlay').classList.remove('show'); _chtProjId=null; }

// ── Charters hub (WORK › Charters rail view) ─────────────────────────────────
// A full-screen list of every project; clicking one opens its charter editor
// (which stacks on top at a higher z-index). Closing the editor reveals the hub.
export function openCharterHub(){
...
function chtRenderHub(){
  ...
  if(!projects.length){
    body.innerHTML='<div class="cht-muted" style="padding:20px">No projects yet — add one on the Portfolio matrix, then open its charter here.</div>';
    return;
  }
  body.innerHTML='<div class="chthub-grid">'+projects.map(p=>{
    ...
    return `<div class="chthub-card" onclick="openCharter(${p.id})" title="Open charter">
...
function chtRenderHeader(){
  const p = projects.find(x=>x.id===_chtProjId);
  const c = chtCharter(); if(!p||!c) return;
...
export function chtShowTab(tab){
  ...
  else if(tab==='decision')  body.innerHTML = chtDecisionTab();
...
// Trade-off SQUARE for the Decision Card. Corners = Features (TL) / Time (TR) /
// Product cost (BL) / Project cost (BR). A single marker sits at the weighted
// centroid of the four stances, so you SEE where the project actually leans —
// and that you can't pull it to every corner at once.
function chtSquareSVG(d){
  const W=300,H=250;
  const corner ={ features:[70,66], time:[230,66], productCost:[70,196], projectCost:[230,196] };
  const labelAt={ features:[70,44], time:[230,44], productCost:[70,222], projectCost:[230,222] };
  const stanceColor={ prioritize:'var(--accent)', balance:'var(--muted)', sacrifice:'var(--danger)' };
  const stances=(d&&d.stances)||{};
  let sx=0, sy=0, sw=0;
  for(const [k] of CHT_DIMS4){ const w=CHT_STANCE_W[stances[k]]??0.28; sw+=w; sx+=w*corner[k][0]; sy+=w*corner[k][1]; }
  const px=sw?sx/sw:150, py=sw?sy/sw:131;
  const warn=chtStanceWarn(d);

  let nodes='';
  for(const [k,label] of CHT_DIMS4){
    const [cx,cy]=corner[k], st=stances[k]||'balance', col=stanceColor[st];
    nodes+=`<circle cx="${cx}" cy="${cy}" r="6" fill="${st==='prioritize'?col:'var(--bg)'}" stroke="${col}" stroke-width="2"/>`;
    const [lx,ly]=labelAt[k];
    nodes+=`<text x="${lx}" y="${ly}" fill="var(--text)" font-size="11.5" font-weight="600" text-anchor="middle">${label}</text>`;
    nodes+=`<text x="${lx}" y="${ly+12}" fill="${col}" font-size="8" font-family="IBM Plex Mono,monospace" text-anchor="middle">${st.toUpperCase()}</text>`;
  }
  return `<svg viewBox="0 0 ${W} ${H}" class="cht-svg" role="img" aria-label="Trade-off square: features, time, product cost, project cost">
    <rect x="70" y="66" width="160" height="130" fill="none" stroke="var(--border)" stroke-width="1.5"/>
    <line x1="150" y1="66" x2="150" y2="196" stroke="var(--border)" stroke-width="0.5" stroke-dasharray="3 3"/>
    <line x1="70" y1="131" x2="230" y2="131" stroke="var(--border)" stroke-width="0.5" stroke-dasharray="3 3"/>
    <circle cx="${px.toFixed(1)}" cy="${py.toFixed(1)}" r="16" fill="var(--accent)" fill-opacity="0.16" stroke="var(--accent)" stroke-width="2.5"/>
    <circle cx="${px.toFixed(1)}" cy="${py.toFixed(1)}" r="3.5" fill="var(--accent)"/>
    ${nodes}
    ${warn?'<text x="150" y="242" fill="var(--warn)" font-size="8.5" text-anchor="middle">⚠ not a trade-off — see note below</text>':''}
  </svg>`;
}
...
// ── Decision tab (the trade-off square) ──────────────────────────────────────
function chtDecisionTab(){
  const c=chtCharter(); const d=c.decision;
  const conflicts=chtConflicts(c);
  const stanceSel=([k,label])=>
    `<label class="cht-f"><span class="cht-l">${label}</span>
      <select class="cht-in" onchange="chtStanceSet('${k}',this.value)">
        ${CHT_STANCES.map(([v,l])=>`<option value="${v}"${d.stances[k]===v?' selected':''}>${l}</option>`).join('')}
      </select></label>`;
  const warn=chtStanceWarn(d);
  ...
  const conflictHTML = conflicts.length
    ...
    : '';
  return `<div class="cht-vizrow">
      <div class="cht-viz"><div class="cht-chart-t">Trade-off square</div>${chtSquareSVG(d)}</div>
      <div class="cht-viz"><div class="cht-chart-t">Cross-functional alignment</div>${chtRadarSVG(c)}</div>
    </div>
    <div class="cht-form">
    <div class="cht-note">Set each dimension's stance. You can't prioritize everything — sacrifice at least one. <b>Product cost</b> = what each unit costs; <b>Project cost</b> = what it costs to develop.</div>
    ${CHT_DIMS4.map(stanceSel).join('')}
    ${warnHTML}
    ${conflictHTML}
...
export function chtStanceSet(dim,value){
  chtCharter().decision.stances[dim]=value; chtSave();
  chtShowTab('decision');   // re-render so the square + conflicts update
}
...
export function chtListAdd(key){ chtCharter().decision[key].push(''); chtSave(); chtShowTab('decision'); }
export function chtListRemove(key,i){ chtCharter().decision[key].splice(i,1); chtSave(); chtShowTab('decision'); }
...
function chtDeckSlidesHTML(){
  ...
  S.push(`<section class="cht-slide"><div class="cht-slide-n">1 / 5</div>
    ...
    <div class="cht-slide-2col">
      <div>${chtSquareSVG(c.decision)}</div>
