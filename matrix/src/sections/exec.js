...
function renderExecTab(){
  ...
  // Two headline visuals side by side (stack on narrow).
  const eco=ecDataset();
  h+='<div style="display:flex;gap:16px;flex-wrap:wrap">'
    +'<div id="xs-sec-spend" style="flex:1;min-width:340px">'+xsSpendSection()+'</div>'
    +'<div style="flex:1;min-width:340px">'+pfSection(t('VALUE vs RISK'), t('Each project by NPV (value) and total risk exposure (Σ RPN). Bubble size = team cost. Top-right = valuable AND risky — your watch list.'), xsBubbleSvg(eco))+'</div>'
    +'</div>';
  ...
  h+='<div id="xs-sec-engage">'+xsEngagementWidget()+'</div>';
...
function xsPinnedItems(){
  ...
  projects.forEach(function(p){
    (p.todos||[]).forEach(function(td){ if(td.execPin) out.push({p:p,kind:'todo',id:td.id,text:td.text||'',done:!!td.done,priority:td.priority||'',due:td.due||'',assignee:asg(td),overdue:!!td.due&&!td.done&&new Date(td.due)<today}); });
    (p.risks||[]).forEach(function(r){ if(r.execPin){ const st=r.status||'open', closed=/closed|done|resolved|mitigated/i.test(st); out.push({p:p,kind:'risk',id:r.id,text:r.desc||'',done:closed,priority:r.priority||'',due:'',assignee:asg(r),overdue:false}); } });
    (p.actions||[]).forEach(function(a){ if(a.execPin){ const st=a.status||'Open', done=/done|complete|closed/i.test(st); const due=a.due||a.end||''; out.push({p:p,kind:'action',id:a.id,text:a.desc||'',done:done,priority:a.priority||'',due:due,assignee:asg(a),overdue:!!due&&!done&&new Date(due)<today}); } });
  });
  return out;
}
...
function xsPinToggleDone(pid,kind,id){
  ...
  saveState();
  const el=G('xs-sec-pins'); if(el) el.innerHTML=xsPlannerPins();
}
...
// ── talent-engagement widget (this week's touchpoints; full planner is TALENT ›
//    Engagement, engagement.js). Its check-off re-renders only this block so it
//    never clobbers the exec tab. ──────────────────────────────────────────────
function xsEngagementWidget(){
  if(typeof tegThisWeekList!=='function') return pfSection(t('TALENT ENGAGEMENT — this week'), '', pfEmpty(t('Engagement planner unavailable.')));
  const list=tegThisWeekList();
  const wk=tegCurWeek();
  const open='<button class="sm" onclick="railGo(event,\'engagement\')" style="border-color:var(--accent);color:var(--accent)">'+t('Open planner →')+'</button>';
  let inner;
  if(!list.length){
    inner=pfEmpty(t('No talents scheduled this week — open the planner to set up a cadence.'));
  } else {
    inner='<div style="display:flex;flex-direction:column;gap:6px">';
    list.forEach(function(x){
      const tcol=TEG_TIER_COLOR[x.en.tier]||'var(--muted)';
      const m=tegActionMeta(x.tp.type);
      inner+='<div style="display:flex;align-items:center;gap:9px;font-size:12px;padding:5px 4px;border-bottom:1px solid var(--border)">'
        +'<input type="checkbox"'+(x.tp.done?' checked':'')+' onchange="xsEngToggle('+x.eng.id+')" style="width:15px;height:15px;cursor:pointer">'
        +'<span style="min-width:150px;color:'+tcol+';font-weight:600'+(x.tp.done?';text-decoration:line-through;opacity:.6':'')+'">'+escH(x.eng.name||'')+'</span>'
        +'<span style="color:var(--muted)">'+m.icon+' '+escH(t(m.label))+'</span></div>';
    });
    inner+='</div>';
  }
  const done=list.filter(function(x){return x.tp.done;}).length;
  return pfSectionShell(t('TALENT ENGAGEMENT — this week'), t('Week of {w} · {d}/{n} done · your key-talent touchpoints.',{w:tegWeekLabel(wk),d:done,n:list.length}), open, inner);
}
// Check off a touchpoint from the exec widget — updates just this block (staying on exec).
function xsEngToggle(id){
  const eng=engineers.find(function(e){return e.id===id;}); if(!eng) return;
  const tp=tegTp(tegEng(eng), tegCurWeek());
  if(tp){ tp.done=!tp.done; saveState(); }
  const el=G('xs-sec-engage'); if(el) el.innerHTML=xsEngagementWidget();
}
