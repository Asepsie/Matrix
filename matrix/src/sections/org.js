...
let _orgLightMode=false;   // white/presentation theme + straight reporting arrows (toolbar toggle)
...
function renderOrgChart(){
  ...
  var L=_orgLightMode;
  var C=L
    ?{bg:'#ffffff',surface:'#ffffff',text:'#1a1a2e',muted:'#5b6b7b',border:'#d4d9e0'}
    :{bg:'#0f0f11',surface:'#18181c',text:'#e8e8ea',muted:'#6b6b78',border:'#2a2a32'};
  var bandRGB=L?'0,0,0':'255,255,255';            // level-band tint base
  var lblCol1=L?'#9aa4b2':'#383848';              // level label
  var lblCol2=L?'#b4bdc8':'#2a2a3a';              // level sub-label
  var arrowCol=L?'#64748b':'#3a3a50';             // reporting-line stroke + arrowhead
  var notRatedFill=L?'#eef2f7':'#1a1a1e';         // detail-mode "NOT RATED" box
  var subtleFill=L?'rgba(0,0,0,0.03)':'rgba(255,255,255,0.03)';
  svg.style.background=L?'#ffffff':'';
  ...
  var s='<defs>'
    ...
    +'<path d="M0,1 L6,3.5 L0,6 Z" fill="'+arrowCol+'"/></marker>'
    +photoDefs+'</defs>';
  ...
  for(var d=0;d<=maxD;d++){
    ...
    s+='<rect x="'+(minX-55)+'" y="'+ly+'" width="'+(tW+40)+'" height="'+lh
      +'" fill="rgba('+bandRGB+','+alpha+')" rx="0"/>';
    var lbl=getLevelLabel(d).toUpperCase();
    s+='<text x="'+(minX-50)+'" y="'+(ly+16)+'" font-size="8" fill="'+lblCol1+'"'
      +' font-family="IBM Plex Mono,monospace" font-weight="bold">'+escH(lbl)+'</text>';
    s+='<text x="'+(minX-50)+'" y="'+(ly+27)+'" font-size="8" fill="'+lblCol2+'"'
  ...
  if(_orgShowArrows) engs.forEach(function(e){
    ...
    s+='<path d="'+pathD+'"'
      +' fill="none" stroke="'+arrowCol+'" stroke-width="1.8" marker-end="url(#orgarr)"/>';
    s+='<path d="'+pathD+'"'
    ...
    if(_orgDetailMode&&!isVacant){
      ...
      if(pot){
        ...
        s+='<rect x="0" y="'+dy+'" width="'+ORG_NW+'" height="17" rx="4"'
          +' fill="'+subtleFill+'" stroke="'+potCol+'" stroke-width="1"/>';
...
// toggles the white/presentation theme (light cards + straight reporting arrows)
...
function orgBuildExportSVG(){
  ...
  var L=_orgLightMode;
  var eBg=L?'#ffffff':'#0f0f11';
  var eSurface=L?'#ffffff':'#18181c';
  var eText=L?'#1a1a2e':'#e8e8ea';
  var eRole=L?'#5b6b7b':'#9a9aaa';
  var eMuted=L?'#5b6b7b':'#6b6b78';
  var eBandRGB=L?'0,0,0':'255,255,255';
  var eLbl1=L?'#9aa4b2':'#3a3a4a';
  var eLbl2=L?'#b4bdc8':'#2a2a34';
  var eArrow=L?'#64748b':'#3a3a50';
  var eNotRated=L?'#eef2f7':'#1a1a1e';

  var s='<svg width="'+W+'" height="'+H+'" viewBox="0 0 '+W+' '+H+'"'
    +' xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">'
    +'<defs>'
    +'<filter id="sh" x="-5%" y="-5%" width="110%" height="130%"><feDropShadow dx="0" dy="3" stdDeviation="4" flood-opacity="'+(L?'0.18':'0.45')+'"/></filter>'
    +(showArrows?'<marker id="arr" markerWidth="7" markerHeight="7" refX="6" refY="3.5" orient="auto"><path d="M0,1 L6,3.5 L0,6 Z" fill="'+eArrow+'"/></marker>':'')
    +photoDefs+'</defs>';
  s+='<rect width="'+W+'" height="'+H+'" fill="'+eBg+'"/>';
  ...
  for(var d=0;d<=maxD;d++){
    ...
    s+='<rect x="'+(minX-60)+'" y="'+lyE+'" width="'+(maxX-minX+100)+'" height="'+(ENH+EVGAP)+'"'
      +' fill="rgba('+eBandRGB+','+(d%2?'0.025':'0.012')+')" rx="0"/>';
    var lbl=(getLevelLabel(d)).toUpperCase();
    s+='<text x="'+(minX-54)+'" y="'+(lyE+18)+'" font-size="9" fill="'+eLbl1+'" font-weight="bold">'+escH(lbl)+'</text>';
    s+='<text x="'+(minX-54)+'" y="'+(lyE+30)+'" font-size="8" fill="'+eLbl2+'">L'+d+'</text>';
  }
  ...
  if(showArrows){
    engs.forEach(function(e){
      ...
      var eD=L?('M'+x1+','+y1+' L'+x2+','+y2):('M'+x1+','+y1+' C'+x1+','+my+' '+x2+','+my+' '+x2+','+y2);
      s+='<path d="'+eD+'"'
        +' fill="none" stroke="'+eArrow+'" stroke-width="2" marker-end="url(#arr)"/>';
    });
  }
  ...
  engs.forEach(function(e){
    ...
    if(isVacant){
      ...
      s+='<rect x="0" y="0" width="'+ENW+'" height="'+ENH+'" rx="9" fill="'+eSurface+'" stroke="'+col+'" stroke-width="1.8" filter="url(#sh)"/>';
    ...
    s+='<text x="'+AX+'" y="62" text-anchor="middle" font-size="11.5" font-weight="bold" fill="'+eText+'">'+escH(nameS)+'</text>';
    if(roleS)s+='<text x="'+AX+'" y="75" text-anchor="middle" font-size="9.5" fill="'+eRole+'">'+escH(roleS)+'</text>';
    ...
    if(meta.length){
      ...
      s+='<text x="'+AX+'" y="88" text-anchor="middle" font-size="8.5" fill="'+eMuted+'">'+escH(metaStr)+'</text>';
    }
    ...
    if(kidsN){
      s+='<circle cx="'+(ENW/2)+'" cy="'+ENH+'" r="10" fill="'+eSurface+'" stroke="'+col+'" stroke-width="1.5"/>';
